...
> IMU / Stereo Camera 系列

本目录是一个 **独立、可直接运行的 Docusaurus 站点**。目前作为 `rdk_doc` 单仓内的骨架，将来会迁出为独立 Git 仓库 `rdk-accessories-docs`。
...
- 站点 `baseUrl` 默认 `/accessories/`；迁出后若放在独立域名根路径下，改成 `"/"`。
